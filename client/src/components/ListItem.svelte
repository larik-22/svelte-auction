...
<div class="relative p-4 pt-8 shadow-md rounded-md flex flex-col items-start justify-items-start flex-grow gap-2">
    ...
    <div class="self-stretch flex flex-col items-start gap-1 pb-4">
        <div class="self-stretch flex justify-between gap-2 items-center">
            <h2 class="text-xl font-medium">{item.name}</h2>
            <p class="text-xs bg-gray-100 px-2 py-1 rounded-full text-gray-700 border border-gray-400">{new Date(item.startDate).toLocaleString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric'})}</p>
        </div>
