<script>
    ...
    const handleBidSubmission = (e) => {
        e.preventDefault();
        // validate if the amount is higher than the highest bid and estimated price
        // don't allow to overbid yourself.
        validateBid();
        if (error) return;
...
<form class="flex flex-col space-y-2">
    ...
    <div class="flex items-stretch space-x-3">
        <input type="number" id="item-input" placeholder="Type your bid..." bind:value={amount}
        ...
        <div class="flex-shrink-0 items-stretch">
            <Button className="h-full" on:click={(e) => handleBidSubmission(e)}
                    disabled={!loggedIn || isAdmin || new Date() > new Date(endDate)} type="submit">Place bid
            </Button>
        </div>
